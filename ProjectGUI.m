...
if flag == 1
    ...
    if get(handles.radiobutton1,'value') % scaling mode
        for i = 1:length(start_t)
            if i == 1 && start_t(1) == 0
    ...
    elseif get(handles.radiobutton2,'value') % duration mode
        for i = 1:length(start_t)
            if i == 1 && start_t(1) == 0
                y(1:Fs*(end_t(1))) = target(1)/(end_t(1)-start_t(1));
            else
                y(Fs*start_t(i):Fs*(end_t(i))) = target(i)/(end_t(1)-start_t(1));   
            end
        end
    end
